...
// The UUID client must use. Change this value to your own UUID.
const userID = '2ea73714-138e-4cc7-8cab-d7caf476d51b';
const uuidBytes = new Uint8Array(userID.replace(/-/g, '').match(/.{1,2}/g).map(byte => parseInt(byte, 16)));
  ...
  try {
    ...
    const om = async ev => {
      ...
      try {
        ...
        if (ms.length < 17) throw new Error('Invalid VLESS header');
        for (let i = 0; i < 16; i++) {
            if (uuidBytes[i] !== ms[i + 1]) {
